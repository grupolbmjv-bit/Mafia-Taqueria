...
export default async function AnalisisPage() {
  ...
  let evolucionCosto: { fecha: string; costo_promedio: number }[] = [];
  let error: string | null = null;
  try {
    const [insumos, subrecetas, recetas, ingredientes, preciosHistoricos, historialRecetas, familias, subfamilias, analyticsLegacy] = await Promise.all([
      getInsumos(),
      getSubrecetas(true),
      getRecetas(true),
      getAllIngredientes(),
    ...
    evolucionCosto = analyticsLegacy?.evolucion_costo || [];
  } catch (e) {
  ...
  return (
    <main className="app-shell py-8">
      ...
      {error ? (
        ...
        <AnalisisDashboard analysis={analysis} dataset={dataset} evolucionCosto={evolucionCosto} />
      )}
    </main>
  );
}
