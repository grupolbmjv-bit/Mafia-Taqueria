...
/** Tarjeta superior 1: insumo mas inflacionario, enriquecida con el impacto en cascada (calculateCostImpact). */
export interface ResumenInsumo extends MoverInsumo {
  recetasAfectadas: number;
  subrecetasAfectadas: number;
  impactoEconomico: number;
}

/** Tarjeta superior 2: subreceta mas afectada, enriquecida con el impacto en cascada (calculateCostImpact). */
export interface ResumenSubreceta extends MoverReceta {
  recetasQueLaUsan: number;
  impactoEconomico: number;
}

/** Tarjeta superior 3: receta mas afectada, con utilidad perdida (derivada de utilidadAnterior/utilidadNueva). */
export interface ResumenReceta extends MoverReceta {
  utilidadPerdida: number;
}

export interface AnalysisData {
  insumoMasInflacionario: ResumenInsumo | null;
  subrecetaMasAfectada: ResumenSubreceta | null;
  recetaMasAfectada: ResumenReceta | null;
...
export function buildAnalysisData(dataset: DatasetCompleto): AnalysisData {
  ...
  // --- Enriquecimiento de las 3 primeras tarjetas del resumen superior, reutilizando siempre calculateCostImpact() (no se duplica logica de costeo). ---
  const insumoTop = moversInsumos[0] || null;
  let insumoMasInflacionario: ResumenInsumo | null = null;
  if (insumoTop) {
    const resultado = calculateCostImpact({ tipo: 'insumo', itemId: insumoTop.id, costoAnterior: insumoTop.costoAnterior, costoNuevo: insumoTop.costoActual, dataset });
    insumoMasInflacionario = {
      ...insumoTop,
      recetasAfectadas: resultado.recetasAfectadas.length,
      subrecetasAfectadas: resultado.subrecetasAfectadas.length,
      impactoEconomico: resultado.impactoEconomico,
    };
  }

  const subTop = moversSubrecetas[0] || null;
  let subrecetaMasAfectada: ResumenSubreceta | null = null;
  if (subTop) {
    const resultado = calculateCostImpact({ tipo: 'subreceta', itemId: subTop.id, costoAnterior: subTop.costoAnterior, costoNuevo: subTop.costoNuevo, dataset });
    subrecetaMasAfectada = {
      ...subTop,
      recetasQueLaUsan: resultado.recetasAfectadas.length,
      impactoEconomico: resultado.impactoEconomico,
    };
  }

  const recTop = moversRecetas[0] || null;
  const recetaMasAfectada: ResumenReceta | null = recTop
    ? { ...recTop, utilidadPerdida: recTop.utilidadAnterior - recTop.utilidadNueva }
    : null;
